...
def importAllCollection():
    img_list = io.ImageCollection('./palms_data')
    return img_list
...
def findLandmarks(image, index, collection):
    ...
    ax5.hist(image.ravel(), bins='auto', histtype='step', color='black')
    # ax5.imshow(roi_rescaled, cmap='gray')
    # ax6.imshow(lbp, cmap='gray')
    ax6.hist(image.ravel(), bins=256, histtype='step', color='black')
